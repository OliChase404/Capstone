...
global book_limit; book_limit = 100000
...
def get_connected_books(book):
    ...
    page = BeautifulSoup(requests.get(book.audible_url).text, 'html.parser')
            ...
            if int(review_rating) >= 4:
                ...
                for reviewer_book in reviewer_books:
                    ...
                    if int(reviewer_book_rating) >= 4:
                        ...
                        if reviewer_book_link == book.audible_url:
                            return
                        get_book_from_audible(reviewer_book_link)
                        found_book = Book.query.filter_by(audible_url=reviewer_book_link).first()
                        if found_book.id == book.id:
                            return
                        new_connection = BookConnection(
                        book_id=book.id,
                        connected_book_id=found_book.id,
                        strength=1
                        )
                        db.session.add(new_connection)
                        db.session.commit()
                        # existing_connection = BookConnection.query.filter_by(book_id=book.id, connected_book_id=found_book.id).first()
                        # if existing_connection:
                        #     print('Existing connection found, strength increased')
                        #     existing_connection.strength += 1
                        # else:
                        #     new_connection = BookConnection(
                        #         book_id=book.id,
                        #         connected_book_id=found_book.id,
                        #         strength=1
                        #     )
                        #     print('New connection found, strength set to 1')
                        #     db.session.add(new_connection)
                        # db.session.commit()
...
if __name__ == '__main__':
    ...
    with app.app_context():
        ...
        # get_book_from_audible('https://www.audible.com/pd/Killing-Floor-Audiobook/B015RQON6I?source_code=BLGORWS0107160001')
